            ...
            <tbody className="divide-y divide-gray-200">
              {storeOwners.map((so) => (
                <tr key={so.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3">
                    <Link href={`/admin/store-owners/${so.id}`} className="text-sm text-blue-600 hover:underline">
                      Xem chi tiết
                    </Link>
                  </td>
                </tr>
          ...
          {Array.from({ length: totalPages }, (_, i) => i + 1).map((p) => (
            <button
              ...
              className={`rounded px-3 py-1.5 text-sm ${
                p === currentPage ? 'bg-blue-600 text-white' : 'border bg-white text-gray-600 hover:bg-gray-50'
              }`}
            >
              {p}
            </button>
