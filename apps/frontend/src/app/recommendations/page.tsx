...
export default function RecommendationsPage() {
  ...
  // Sync URL → state on searchParams change
  useEffect(() => {
    const raw = searchParams.get('tags');
  ...
  // Fetch recommendations when selectedIds or page changes
  useEffect(() => {
  ...
  return (
    <main className="mx-auto max-w-4xl px-4 py-8">
      ...
      <p className="mb-8 text-gray-500">
        Chọn các nhãn sở thích để nhận gợi ý món ăn phù hợp với bạn.
      </p>

      <div className="grid gap-8 lg:grid-cols-[280px_1fr]">
        {/* Tag selector */}
        <aside className="rounded-xl border border-gray-100 bg-white p-5 shadow-sm h-fit">
          <TagSelector
            groups={groups}
          ...
          {selectedIds.length === 0 ? (
            <div className="rounded-xl border border-gray-100 bg-white py-16 text-center">
              ...
              <p className="mt-1 text-sm text-gray-400">
                Chọn các nhãn bên trái để nhận gợi ý món ăn phù hợp
              </p>
            </div>
          ) : (
            <>
